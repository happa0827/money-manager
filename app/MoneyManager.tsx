...
const MoneyManager = () => {
  ...
  const ViewSummaryButton = ({ transactions }: { transactions: any[] }) => {
    ...
    return (
      <button onClick={handleClick} className="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600">
        月別収支を見る
      </button>
    );
  };
  ...
  <div className="absolute left-4 top-4">
    <LogoutButton />
          ...
          <CardTitle className={`text-2xl text-center ${darkMode ? 'text-white' : 'text-black'}`}>家計簿アプリ</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-center mb-8">
            <div className={`text-3xl font-bold flex items-center justify-center ${darkMode ? 'text-white' : 'text-black'}`}>
  ...
  {transactions.length === 0 ? (
    ...
    transactions.map((t) => (
      <Card key={t.id} className={darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white'}>
        <CardHeader>
          <CardTitle className={darkMode ? 'text-white' : 'text-black'}>{t.description}</CardTitle>
