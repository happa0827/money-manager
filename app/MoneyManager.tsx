...
import { useTheme } from '@/contexts/ThemeContext'; // ←追加
...
const MoneyManager = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const totalBalance = transactionsWithFormatted.reduce(
  ...
  const handleTransaction = async (type: 'income' | 'expense') => {
    ...
    const supabaseInsertData = {
      ...
      formatteddate: newTransaction.formattedDate,
      user_id: user.id, // ユーザーIDを追加
    };

    try {
      const { data, error } = await supabase.from('transactions').insert([supabaseInsertData]);
      ...
      setBalance(prev => (type === 'income' ? prev + newAmount : prev - newAmount));
      setAmount('');
      setDescription('');
    } catch (error) {
  ...
  const handleReset = async () => {
    ...
    if (window.confirm('全てのデータをリセットしてもよろしいですか？')) {
      try {
        const { error } = await supabase.from('transactions').delete().eq('user_id', user.id); // ユーザーIDでフィルタリング
  ...
  const handleLoadFromFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async event => {
      try {
        ...
        // Supabaseに挿入
        const { data, error } = await supabase.from('transactions').insert(formattedTransactions);
    ...
    <div className={`${darkMode ? 'bg-gray-900' : 'bg-white'} min-h-screen`}>
      <Card className={darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white'}>
        <CardHeader className="relative">
          ...
          <CardTitle className={`text-2xl text-center ${darkMode ? 'text-white' : 'text-black'}`}>
            家計簿アプリ
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="space-y-4">
            ...
            <div className="mt-4 text-center">
              <button onClick={handleReset} className="text-red-600 p-2 hover:bg-red-100 rounded">
                データをリセット
              </button>
            </div>
          </div>
